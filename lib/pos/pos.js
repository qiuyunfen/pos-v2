...
function analysisBarcodes(tests) {
    return tests.map(barcode => {
        let [Itembarcode, count] = barcode.split('-');
        count = count !== undefined ? parseFloat(count) : 1;
        return {barcode: Itembarcode, count: count};
    });
}
...
function findItem(barcode, cartBarcodes) {
    for(const item of cartBarcodes) {
...
function getCartItems(cartBarcodes) {
    ...
    for(const item of allItems) {
        for(const cartItem of cartBarcodes) {
